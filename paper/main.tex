...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/morph.eps}
...
including Bayesian Model Combination (BMC)
...
$i > 21$.
BMC clearly outperforms TPC, the benchmark base classifier,
...
We also show the star and galaxy purity values as a function of
...
both TPC and morphological separation over all redshift range.
...
both TPC and morphological separation over all $g-r$ color range.  
...
\begin{table*}
  \caption{A summary of the classification performance metrics
           for the four individual methods
           and the four different classification combination methods
           for when the training data set consists of
...
when there is a high-quality training set,
it now chooses various methods and outperforms all base classifiers.
While the performance of the stacking technique is comparable
to that of BMC in Section~\ref{section:rich_training},
...
Compared to Figure~\ref{fig:purity_mag},
the purity of TPC suffers a significant decrease in purity.
...
only a small number of objects in regions of particular parameter space,
...
and does not assign a star probability $P(S) \ga 0.95$ to any true galaxies,
while both morphological separation and TPC techniques
...
The combination techniques that we have demonstrated in this work uses
...
even when training is restricted to $i < 20.0$.
...
and BMC performs worse than morphological separation
...
of the stellar fraction map (top right)
have bright magnitudes $i \la 20$ in the $i$-band magnitude map (top left)
and blue colors $g-r \la 0.5$ in the $g-r$ color map (bottom right).
On the other hand, the darker (fainter magnitude) cells
in the lower left corner of the $i$-band magnitude map
...
a sufficient number of the minority class in the training data set
